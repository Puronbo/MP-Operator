...
# Analyze for L(s,chi)
print("Analyzing L(s,chi_{-4})...")
F_chi, FtF_chi, x_chi = build_F_and_FtF(chi_zeros_all[:20])
chi_analysis = analyze_FtF(FtF_chi, "L(s,chi)")
print(f"L(s,chi): Participation ratio = {chi_analysis['pr']:.2f}")
...
ax3.set_title('Re(F*F) for unfolded ζ zeros')
...
ax4.semilogy(zeta_analysis['evals_real'], 'bo-', label='ζ(s)')
ax4.semilogy(chi_analysis['evals_real'], 'rs--', label='L(s,χ₋₄)')
ax4.semilogy(unfolded_analysis['evals_real'], 'g^-', label='ζ unfolded')
...
ax6.plot(lags, zeta_analysis['off_diag_mag'][1:], 'bo-', label='ζ(s)')
ax6.plot(lags, chi_analysis['off_diag_mag'][1:], 'rs--', label='L(s,χ₋₄)')
ax6.plot(lags, unfolded_analysis['off_diag_mag'][1:], 'g^-', label='ζ unfolded')
...
ax7.hist(zeta_analysis['evals_real'], bins=20, alpha=0.7, label='ζ(s)', color='blue')
ax7.hist(chi_analysis['evals_real'], bins=20, alpha=0.7, label='L(s,χ₋₄)', color='red')
ax7.hist(unfolded_analysis['evals_real'], bins=20, alpha=0.7, label='ζ unfolded', color='green')
...
ax9.set_ylabel('Condition number (λ_max/λ_min)')
...
plt.tight_layout()
plt.savefig('ftf_analysis.png', dpi=150)
print("\nSaved ftf_analysis.png")
...
print(f"Zeta(s) unfolded (equally spaced γ):")
...
print("- For actual zeta zeros: PR ≈ 5-7 out of N=20 → only ~30% effective DOF")
print("- For unfolded (equally spaced) zeros: PR ≈ 12-14 out of N=20 → ~65% effective DOF")
print("This shows that the actual repulsion of zeros (GUE statistics) increases correlations,")
print("making F*F more concentrated in fewer eigenmodes compared to equally spaced zeros.")
print("")
print("For the Riemann Hypothesis to hold via the isometry strategy, we would need")
print("F*F to be close to a scalar multiple of the identity (PR ≈ N, condition number ≈ 1).")
