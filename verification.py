...
def main():
    ...
    print(f"Injected off-line zero: ρ = {rho_off}")
    ...
    if increasing or diff_off[-1] > diff_off[0] * 10:  # Allow some fluctuation but expect growth
        print("✓ PASS: Discrepancy grows with x (indicates sensitivity to off-line zeros)")
    else:
    ...
    for i in range(min(3, n_zeros)):  # Check first 3 eigenvectors
        ...
        # Compare to sine wave: sin(2π * k * x_indices / n_zeros) for k=1,2,...
        best_corr = 0
