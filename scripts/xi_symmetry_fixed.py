...
t_min, t_max = 0, 30  # reduce to avoid too much computation
resolution = 300  # points in each direction
...
for t in t_critical:
    s = mp.mpc(0.5, t)
    xi_crit.append(xi(s))
xi_crit = np.array(xi_crit)
ax5.plot(t_critical, xi_crit.real, 'b-', label='Re')
ax5.plot(t_critical, xi_crit.imag, 'r--', label='Im')
...
plt.tight_layout()
plt.savefig('xi_symmetry_fixed.png', dpi=150)
print("Saved xi_symmetry_fixed.png")
...
print(f"Mean: {np.mean(errs):.2e}, Max: {np.max(errs):.2e}")
...
for sigma,t in test_points:
    ...
    print(f"  s={sigma}+{t}i: err={err:.2e}")
print(f"Mean: {np.mean(conj_errs):.2e}, Max: {np.max(conj_errs):.2e}")